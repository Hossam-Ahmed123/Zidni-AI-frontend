  ...
  <v-layout class="rounded rounded-md h-screen">
    ...
    >
      <div class="d-flex align-center justify-center pa-4">
        <!-- Logo placeholder -->
         <div class="text-h6 font-weight-bold text-primary">EDTECH</div>
      </div>
      ...
      <template v-slot:append>
        <div class="pa-4">
           <v-btn block variant="tonal" color="primary" v-if="!rail" @click="toggleRail">
    ...
    <v-app-bar flat border="b" color="background" class="px-4">
      ...
      <!-- Notifications -->
      <v-btn icon variant="text" class="mr-2" to="/teacher/notifications">
      ...
      <v-menu min-width="200px" rounded offset-y>
        ...
        <v-card width="250">
          ...
          <v-list density="compact" nav>
            <v-list-item prepend-icon="pi pi-wallet" :title="t('nav.teacherPaymentSettings')" value="payments" to="/teacher/payments/manual-settings"></v-list-item>
            <v-list-item prepend-icon="pi pi-cog" :title="t('nav.settings')" value="settings" to="/teacher/settings"></v-list-item>
...
const navContext = computed(() => ({
  ...
  assistantPermissions: auth.assistantPermissions ? new Set(auth.assistantPermissions) : null,
